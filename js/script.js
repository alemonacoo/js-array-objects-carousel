...
// settings
const NUM_IMAGES = imagesObject.length;
const CHANGE_IMAGE_DELAY = 5;

const urlsArray = createUrlsArray(NUM_IMAGES);
console.log(urlsArray);


let activeIndex = 0;
buildCarousel(urlsArray, activeIndex);
...
// Functions:
function createUrlsArray(numImages){
    const images = [];
    for(let i = 0; i < numImages; i++){
        images.push(imagesObject[i].url);
    }
    return images;
}
...
function moveCarouselForward(){
    ...
    activeIndex = activeIndex < urlsArray.length -1 ? activeIndex +1 : 0 ;
    buildCarousel(urlsArray, activeIndex);
...
function moveCarouselPrevious(){
    ...
    activeIndex = activeIndex > 0 ? activeIndex -1 : urlsArray.length -1 ;
    buildCarousel(urlsArray, activeIndex);
    idInterval = setInterval(moveCarouselForward, CHANGE_IMAGE_DELAY * 1000);
}